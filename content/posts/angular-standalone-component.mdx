...
Standalone components, a new feature introduced in `Angular v14`, **are not part of an NgModule and define their
dependencies (like module imports) directly**. However, this doesn't necessarily enable us to do anything that we
couldn't do with existing Angular; it just changes some of the architecture.

React, Vue, and other frameworks don't have the NgModule concept. They can import components and use them directly. The
Angular community enables this feature so React developers can get hands-on experience easily.
...
Creating a project-wide place for all common things, like widgets or pipes to transform values, makes sense. You can
...
Right now, standalone components don't really reduce the quantity of work. Yes, you don't need the module file anymore,
but now each component needs each of these pieces, and in many cases, we're seeing that we have to duplicate things. We
have to import the router module multiple times or the forms module multiple times, rather than just importing it once
inside the module and having it be available to the component. So right now, standalone components aren't reducing the
quantity of work that we have to do.

### Lower learning curve?

One thing that standalone components may offer is a lower learning curve. Modules can be confusing to people who are new
to Angular and learning Angular, so having an application that doesn't have a route module might be slightly easier for
somebody to learn.
