...
Bad example: Why can't we use `withLatestFrom`? Because we are not sure if productCategories$ is returned earlier. If
products$ returns earlier, at the emit time, productCategories$ is not complete, it never emits a value, and the output
...
Category dropdown selection will trigger the emission of the categoryId.
