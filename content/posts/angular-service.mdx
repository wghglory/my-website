...
Why? To avoid duplicate token names.
...
Suppose we have a logger service, and we'd like to create a SpecialLoggerService that extends LoggerService.
...
values, which provides a useful way of accessing common configuration settings.
...
Using either token will result in the dependency being resolved to the same value.
...
Use this LogService first; if not found, search up the hierarchy until reaching the root.
