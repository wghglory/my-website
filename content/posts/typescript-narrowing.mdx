...
Let's imagine we're trying to encode shapes like circles and squares. Circles keep track of their
radii and squares keep track of their side lengths. We'll use a field called `kind` to identify which shape we're dealing
...
We can write a `getArea` function that applies the right logic based on whether it's dealing with a circle or square. We'll
...
But this doesn't feel ideal. We had to use non-null assertions (`!`) to convince
the type-checker that `shape.radius` was defined, but those assertions are error-prone if we start to move code around. Additionally,
...
The problem with this encoding of `Shape` is that the type-checker doesn't have any way to know whether `radius`
or `sideLength` are present based on the `kind` property. We need to communicate what _we_ know to the type checker.
With that in mind, let's take another approach to defining `Shape`.
...
Checking whether the `kind` property was `"circle"` eliminated every type in `Shape` that didn't have a `kind` property
with the type `"circle"`. That narrowed `shape` down to the type `Circle`.

The same checking works with `switch` statements as well. Now we can write our complete `getArea` without any
non-null assertions.
...
For example, adding a `default` case to our `getArea` function that tries to assign the shape to `never` will raise an error when
